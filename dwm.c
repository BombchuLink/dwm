...
static void sendnextmon(const Arg *arg);
...
applyrules(Client *c)
{
	...
	for (i = 0; i < LENGTH(rules); i++) {
		...
		&& (!r->instance || strstr(instance, r->instance)))
		{
			...
			c->tags |= r->tags;
			if (r->isfloating) {
				//TODO figure out WHY floatx and floaty are ignored but floatw and floath are not
